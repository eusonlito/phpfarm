...
# Default-Stop:      0 1 6
# Short-Description: starts php-fpm-$VERSION
...
PATH=/sbin:/usr/sbin:/bin:/usr/bin:INSTDIR/sbin:INSTDIR/bin
DESC="PHP-VERSION FastCGI Process Manager"
NAME=php-fpm-VERSION
DAEMON=INSTDIR/sbin/php-fpm
CONFFILE=ETCDIR/php-fpm.conf
DAEMON_ARGS="--daemonize --fpm-config $CONFFILE"
CONF_PIDFILE=$(sed -n 's/^pid[ =]*//p' $CONFFILE)
PIDFILE=${CONF_PIDFILE:-INSTDIR/var/run/php-fpm-VERSION.pid}
TIMEOUT=30
SCRIPTNAME=/etc/init.d/$NAME

# Exit if the package is not installed
if [ ! -x "$DAEMON" ]; then
    echo "Daemon $DAEMON not exists"
    exit 1
fi
